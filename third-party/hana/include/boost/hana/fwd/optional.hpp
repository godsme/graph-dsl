...
BOOST_HANA_NAMESPACE_BEGIN
    ...
    //! @relates hana::optional
    //!
    //! Specifically, `down_stream_maybe` takes a default value, a function and an
    ...
    //! of the form `just(x)`. In that case, the result returend by `down_stream_maybe`
    //! is the result of `f`.
    //!
    //! @param m
    //! An optional value.
    ...
    //! Example
    //! -------
    //! @include example/optional/down_stream_maybe.cpp
#ifdef BOOST_HANA_DOXYGEN_INVOKED
    constexpr auto down_stream_maybe = [](auto&& default_, auto&& f, auto&& m) -> decltype(auto) {
        if (m is a just(x)) {
